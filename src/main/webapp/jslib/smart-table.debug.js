  ...
  .directive('stPagination', ['stConfig', function (stConfig) {
    return {
      ...
      link: function (scope, element, attrs, ctrl) {
        ...
        scope.currentPage = 1;
        scope.pages = [];
        scope.totalCountPages =[];
       //scope.records = scope.order;
        ...
        var y;
          ...
          for(y = 1;y<= scope.numPages; y++){
                scope.totalCountPages.push(y);
          }
          for (i = start; i < end; i++) {
            scope.pages.push(i);
          }
        ...
        //view -> table state
        scope.selectPage = function (page,target) {
              var test = page;
               if(target != undefined){
                 var tt = target.target;
                // console.log(target.target.parentElement);
                 $("#landingPaginationID li.ng-scope").removeClass("active");
                 $(target.target.parentElement).addClass("active");
                 $("#paginationDropDown option[value = '"+(page-1)+"']").attr('selected', true).siblings('option').removeAttr('selected');
