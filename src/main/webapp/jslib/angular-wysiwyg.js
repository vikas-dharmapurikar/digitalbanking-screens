...
angular.module('wysiwyg.module', ['colorpicker.module'])
    .directive('wysiwyg', function($timeout, wysiwgGui, $compile) {
        return {
            ...
            link: function(scope, element, attrs, ngModelController) {
                ...
                console.log(scope.viewAccess);
                var textarea = element.find('div.wysiwyg-textarea');

                
                
                scope.$watch(attrs.viewAccess,function(value) {
							var val = value || null;
							if (val) {
								scope[attrs.viewAccess] = scope.$eval(attrs.viewAccess);
								
								scope.viewAccess = scope[attrs.viewAccess];
								if(scope.viewAccess){
									$('.wysiwyg-menu *').attr('disabled', true);
								}
                ...
                textarea.on('input keyup paste mouseup', function(event) {
                      ...
                      //made the change to set value only when value being changed is different.
                      //The reason for this change: when click on textarea the form become dirty even though
                      //there is no change made. 
                      
                        scope.oldValue =  (scope.oldValue == undefined?"":scope.oldValue);
                        if (scope.oldValue != html) {
                ...
                ngModelController.$render = function() {
                    ...
                    //storing the rendered value in a variable.
                    scope.oldValue = ngModelController.$viewValue;
                };
    ...
    .factory('wysiwgGui', function() {
        ...
        var getMenuTextArea = function() {
            return '<div id="{{textareaId}}" ng-disabled="{{viewAccess}}" ng-attr-style="resize:vertical;height:{{textareaHeight || \'80px\'}}; overflow:auto" contentEditable="{{!viewAccess}}" class="{{textareaClass}} wysiwyg-textarea" rows="{{textareaRows}}" name="{{textareaName}}" required="{{textareaRequired}}" placeholder="{{textareaPlaceholder}}" ng-model="value"></div>';
        }
